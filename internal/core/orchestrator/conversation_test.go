...
// TestDisableFinishOnNoToolRelaxesGateMidWork proves the EXPERIMENTAL
// continuation.disableFinishOnNoTool flag stops a tool-less turn from ending an
// in-flight run once at least one tool has been used. sequenceBridge calls a
// tool on turn 1 then narrates ("continued") on every later turn with no tool —
// the MiniMax "announce-then-act" shape. With the gate active (default) the run
// would stop at turn 2; with the flag on it keeps going until the inference
// budget, so we assert it ran to the cap instead of finishing early.
func TestDisableFinishOnNoToolRelaxesGateMidWork(t *testing.T) {
	fake := &sequenceBridge{}
	...
	oc.Continuation.DisableFinishOnNoTool = true
	oc.Continuation.MaxInferenceTurns = 4 // resource cap still bounds the relaxed run
	snap := providerSnapshot{
	...
	close(out)
	if err == nil || !strings.Contains(err.Error(), "inference-turn budget") {
		t.Fatalf("expected run to continue to the turn cap (gate relaxed), got err = %v", err)
	}
	if len(fake.requests) != 4 {
		t.Fatalf("requests = %d, want 4 (gate should not finish at turn 2)", len(fake.requests))
	}
}

// TestDisableFinishOnNoToolStillFinishesPureTextChat proves the experimental
// flag does NOT hang an ordinary chat reply: a turn that uses zero tools all
// run (thinkingBridge: reason + final text, no tool) still finishes in one
// turn, because the relaxation only applies once a tool has been used.
func TestDisableFinishOnNoToolStillFinishesPureTextChat(t *testing.T) {
	orch := &Orchestrator{memoryDir: t.TempDir(), vision: make(map[string]bool), active: make(map[string]*activeRun)}
	oc := config.DefaultOrchestratorConfig()
	oc.Continuation.DisableFinishOnNoTool = true
	snap := providerSnapshot{
	...
	close(out)
	if err != nil {
		t.Fatalf("pure-text chat should finish cleanly, got err = %v", err)
	}
	if result.String() != "final answer" {
		t.Fatalf("result = %q, want %q", result.String(), "final answer")
	}
}
