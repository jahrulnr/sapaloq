# 9router → gh/gpt-5-mini (stream)

> Last updated: 2026-07-02 (characterize suite)

Live characterization via `test/9router` — raw `net/http` POST to 9router chat/completions (no SapaLOQ orchestrator). Mode: **`stream`** (`stream: true`). Weather scenario: `get_weather` fake tool round-trip. Raw capture: `/apps/workspace/sapaloq/tmp/9router/gh-gpt-5-mini-stream.jsonl` (4 records). Transcript: `/apps/workspace/sapaloq/tmp/9router/gh-gpt-5-mini-stream.md`.
...
```text
upstream status 400: {"error":{"message":"[github/gpt-5-mini] [400]: {\"error\":{\"message\":\"The requested model is not supported.\",\"code\":\"model_not_supported\",\"param\":\"model\",\"type\":\"invalid_request_error\"}}\n (reset after 30s)"}}
```
