...
func nrouterEnabled() bool { return provider.enabled() }

func nrouterAPIKey() string { return provider.apiKey() }

func nrouterEndpoint() string { return provider.endpoint() }

func requireNrouterModels(t *testing.T) []ModelSpec {
	return provider.requireModels(t)
}

func nrouterRawDir(t *testing.T) string { return provider.tmpDir(t) }

func nrouterRawPath(t *testing.T, model string, stream StreamMode) string {
	return provider.rawPath(t, model, stream)
}

func nrouterTranscriptPath(t *testing.T, model string, stream StreamMode) string {
...
func sniffParser(model string) string {
	...
	for _, marker := range []string{"kimi", "moonshot", "kr/"} {
		if strings.Contains(lower, marker) {
			return "kimi"
		}
	}
	for _, marker := range []string{"codex", "cursor", "cu/"} {
		if strings.Contains(lower, marker) {
			return "openai"
		}
	}
