...
// writeProviderCharacterizationDoc materializes docs/providers/9router-<model>-<mode>.md
...
func renderProviderDoc(spec ModelSpec, stream StreamMode, report CharacterReport, rawPath string, eventCount int) string {
	...
	b.WriteString("}\n```\n\n")
	fmt.Fprintf(&b, "%s exposes OpenAI-compatible `/v1/chat/completions` locally (default `127.0.0.1:20128`). Model slugs are gateway aliases (e.g. `cu/default`, `codex-cursor`, `kr/claude-sonnet-4.5`). Prefer `parser: \"openai\"` + bearer; use `parser: \"kimi\"` for Kimi-routed slugs.\n\n", provider.DisplayName)
