...
// rawRecord is one line in tmp/blackbox/*.jsonl — unmodified capture from the raw HTTP probe.
...
// runRawCharacterize performs a two-turn Blackbox chat/completions probe using
...
func runRawCharacterize(t *testing.T, spec ModelSpec, stream StreamMode) (rawPath string, report CharacterReport) {
	t.Helper()
	start := time.Now()
	path := blackboxRawPath(t, spec.Model, stream)
	transcriptPath := blackboxTranscriptPath(t, spec.Model, stream)
...
func chatTurn(ctx context.Context, client *http.Client, spec ModelSpec, messages []chatMessage, opts turnOpts, stream StreamMode, write func(rawRecord)) (turnResult, error) {
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodPost, blackboxEndpoint(), bytes.NewReader(body))
	if err != nil {
		return turnResult{}, err
	}
	req.Header.Set("Authorization", "Bearer "+blackboxAPIKey())
