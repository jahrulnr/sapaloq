...
// rawRecord is one line in tmp/llamacpp/*.jsonl — unmodified capture from the raw HTTP probe.
...
// runRawCharacterize performs a two-turn Llamacpp chat/completions probe using
...
func runRawCharacterize(t *testing.T, spec ModelSpec, stream StreamMode) (rawPath string, report CharacterReport) {
	t.Helper()
	start := time.Now()
	path := llamacppRawPath(t, spec.Model, stream)
	transcriptPath := llamacppTranscriptPath(t, spec.Model, stream)
	...
	client := &http.Client{Timeout: provider.clientTimeout()}
	if err := provider.preloadModel(ctx, spec.Model); err != nil {
		write(rawRecord{
			At:    time.Now().UTC(),
			Phase: "model_preload_warning",
			Payload: mustJSON(map[string]any{
				"model": spec.Model,
				"error": err.Error(),
			}),
	...
	messages = append(messages, chatMessage{
		Role:      "assistant",
		Content:   "",
		ToolCalls: []wireToolCall{{ID: toolID, Type: "function", Function: wireToolFunction{Name: tc.Name, Arguments: tc.Arguments}}},
	})
	messages = append(messages, chatMessage{
		Role:       "tool",
		ToolCallID: toolID,
...
func chatTurn(ctx context.Context, client *http.Client, spec ModelSpec, messages []chatMessage, opts turnOpts, stream StreamMode, write func(rawRecord)) (turnResult, error) {
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodPost, llamacppEndpoint(), bytes.NewReader(body))
	if err != nil {
		return turnResult{}, err
	}
	req.Header.Set("Content-Type", "application/json")
	if key := llamacppAPIKey(); key != "" {
