...
// writeProviderCharacterizationDoc materializes docs/providers/llamacpp-<model>-<mode>.md
...
func renderProviderDoc(spec ModelSpec, stream StreamMode, report CharacterReport, rawPath string, eventCount int) string {
	...
	if auth == "" {
		if provider.apiKey() != "" {
			auth = "bearer (optional LLAMACPP_API_KEY / LLAMA_API_KEY)"
		} else {
			auth = "none (local llama-server default)"
		}
	}
	...
	b.WriteString("  \"driver\": \"llama-cpp\",\n")
	...
	if spec.Parser != "" {
		...
		b.WriteString("  \"parser\": \"openai\",\n")
	}
	if spec.AuthScheme != "" {
		fmt.Fprintf(&b, "  \"authScheme\": \"%s\",\n", spec.AuthScheme)
	} else {
		b.WriteString("  \"authScheme\": \"bearer\",\n")
	}
	...
	b.WriteString("}\n```\n\n")
	fmt.Fprintf(&b, "%s is OpenAI-shaped (`/v1/chat/completions`). Use `llama-cpp` against direct llama-server (default port 8080). For agent-sidecar WebUI/Ollama paths, point at `http://127.0.0.1:16283` separately — chat completions there may inject system layers.\n\n", provider.DisplayName)
...
func verdictParagraph(_ ModelSpec, r CharacterReport) string {
	if r.Error != "" && !r.ToolSucceeded {
		if strings.Contains(r.Error, "tool_choice") {
	...
	if r.ToolSucceeded && r.AnswerMentionsWeather {
		...
		case probeSupportNo:
			parts = append(parts, "**Leave `reasoningEffort` unset** on this route.")
		}
		switch r.ThinkingSupport {
		case probeSupportYes:
			parts = append(parts, "`thinking` probe accepted.")
		case probeSupportNo:
			parts = append(parts, "**Omit `thinking`** on this route.")
		}
		switch r.ToolChoiceSupport {
		case probeSupportYes:
			parts = append(parts, "`tool_choice: auto` accepted on this route.")
		case probeSupportNo:
			parts = append(parts, "**Omit `tool_choice`** on this route (tools-only).")
		}
