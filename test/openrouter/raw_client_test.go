...
// rawRecord is one line in tmp/openrouter/*.jsonl — unmodified capture from the raw HTTP probe.
...
// runRawCharacterize performs a two-turn OpenRouter chat/completions probe using
...
func runRawCharacterize(t *testing.T, spec ModelSpec, stream StreamMode) (rawPath string, report CharacterReport) {
	t.Helper()
	start := time.Now()
	path := openRouterRawPath(t, spec.Model, stream)
	transcriptPath := openRouterTranscriptPath(t, spec.Model, stream)
