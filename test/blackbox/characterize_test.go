...
// TestCharacterizeNonNativeToolRoundTrip probes Blackbox chat/completions directly
...
//
// If upstream rejects tool_choice, the probe retries with tools only.
//
// Gated: SAPALOQ_BLACKBOX_CHARACTERIZE_E2E=1 + BLACKBOX_API_KEY + BLACKBOX_MODELS.
func TestCharacterizeNonNativeToolRoundTrip(t *testing.T) {
	specs := requireBlackboxModels(t)
