...
// Config holds Blackbox characterize-suite provider settings (env, paths, defaults).
type Config struct {
	Name                   string
	DisplayName            string
	GateEnv                string
...
// provider is the Blackbox characterize config for this package.
var provider = Config{
	Name:                   "blackbox",
	DisplayName:            "Blackbox",
	GateEnv:                "SAPALOQ_BLACKBOX_CHARACTERIZE_E2E",
	CredEnv:                "BLACKBOX_API_KEY",
	ModelsEnv:              "BLACKBOX_MODELS",
	EndpointEnv:            "BLACKBOX_ENDPOINT",
	DefaultEndpoint:        "https://api.blackbox.ai/v1/chat/completions",
	TmpSubdir:              "blackbox",
	DocKeyPrefix:           "blackbox",
	ConfigKeyPrefix:        "blackbox",
	CredentialsEnvDefault:  "BLACKBOX_API_KEY",
	CredentialsEnvOverride: "BLACKBOX_CREDENTIALS_ENV",
	MakeTarget:             "blackbox-characterize",
	TestDir:                "test/blackbox",
...
func (c Config) apiKey() string {
	return strings.TrimSpace(os.Getenv(c.credentialsEnv()))
}
...
func (c Config) requireModels(t *testing.T) []ModelSpec {
	t.Helper()
	if !c.enabled() {
		t.Skipf("set %s=1 (+ %s + %s) to run the %s characterize suite", c.GateEnv, c.credentialsEnv(), c.ModelsEnv, c.DisplayName)
	}
	if c.apiKey() == "" {
		t.Skipf("%s is empty - skipping %s characterize suite", c.credentialsEnv(), c.DisplayName)
	}
