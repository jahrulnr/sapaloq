# Blackbox → anthropic/claude-3.5-haiku (nostream)

> Last updated: 2026-07-02 (characterize suite)

Live characterization via `test/blackbox` — raw `net/http` POST to Blackbox chat/completions (no SapaLOQ orchestrator). Mode: **`nostream`** (`stream: false`). Weather scenario: `get_weather` fake tool round-trip. Raw capture: `/apps/workspace/sapaloq/tmp/blackbox/anthropic-claude-3.5-haiku-nostream.jsonl` (4 records). Transcript: `/apps/workspace/sapaloq/tmp/blackbox/anthropic-claude-3.5-haiku-nostream.md`.
